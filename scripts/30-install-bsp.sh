...
write_armbian_release_fallback() {
    [ ! -f "${ROOTFS_DIR}/etc/armbian-release" ] || return 0

    ${SUDO} tee "${ROOTFS_DIR}/etc/armbian-release" >/dev/null <<EOF_ARMBIAN_RELEASE
# Generated by EasePi-R2 BSP image builder when armbian-bsp-cli did not provide one.
BOARD=${BOARD}
...
BSP_CLI_DEBS=(/tmp/bsp/armbian-bsp-cli-*.deb)
